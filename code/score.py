...
from code.Const import C_ORANGE, SCORE_POS, C_GREEN, MENU_OPTION, C_INDIGO, C_BLACK, C_GOLD, C_CYAN
...
class Score:
    ...
    def save(self, game_mode: str, player_score: list[int]):
        pygame.mixer_music.load('./asset/Score.ogg')
        pygame.mixer_music.play(-1)
        db_proxy = DBProxy('DBScore')
        name = ''
        while True:
            self.window.blit(source=self.surf, dest=self.rect)
            self.score_text(48, 'Victory!', C_CYAN,SCORE_POS['Title'])
    ...
    def show(self):
        ...
        self.score_text(22, 'NAME                 SCORE                    DATE      ',
                        C_CYAN, SCORE_POS['Label'])
        db_proxy = DBProxy('DBScore')
